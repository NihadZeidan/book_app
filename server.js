...
app.post('/searches/show',resultHandler)
...
function resultHandler(req,res) {
 ...
 superAgent.get(url).query(searchQuery).then( (result)=> {return result.body.items.map(
   book => {
    ...
    return newBook;
     ...
     res.render('searches/show', {UserBooks: resultNew.slice(0,11)})
 }
 )
}
...
this.image_url = dataBook.volumeInfo.imageLinks;
}
